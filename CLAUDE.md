...
- **Dual test-environment detection:** Both `MurmurApp.init()` and `AppDelegate.setup()` check for test hosts using `ProcessInfo.processInfo.environment["XCTestBundlePath"] != nil || NSClassFromString("XCTest") != nil`. `MurmurApp` uses in-memory `ModelContainer` in test environments to prevent SwiftData crashes. `AppDelegate.setup()` returns early to skip GUI/hardware init. The `XCTestBundlePath` env var alone is unreliable on macOS 26 beta.
...
- 266+ tests across 22 MurmurCore suites + 13 MurmurTests suites: NoteStoreService, NoteStoreEdgeCases, PersonalDictionaryService, MockSpeechEngine, OnboardingPermissionCoordinator, ThreadingSafety, AIEditorTypes, EditingPipeline, CommandMode, FillerWord, FillerWordEdgeCases, Backtrack, BacktrackEdgeCases, FallbackChain, ClaudeProvider, OpenAIProvider, ProviderErrorHandling, ProviderSystemPrompt (MurmurCore: 236), plus DictationViewModelRace, EarlyInjection, TextInjection, MockSpeechEngineRace, AppDelegateSetup, TriggerKey, ToggleMaxDuration, SessionRecovery, KeychainService, KeychainEdgeCases, AIPipelineIntegration, MenuBarPopover, NoteExporter (MurmurTests: 13 suites)
- All tests use in-memory `ModelContainer`, `MockSpeechEngine`, or `BackgroundCallbackPermissionProvider`
- Tests cover: CRUD, search, trash/restore, auth flows, mic permission, session lifecycle, event streaming, error types, threading safety, onboarding permission flow, race conditions, early injection (snapshot text, tail text update, empty snapshot fallback, no double injection), text injection into TextEdit, TriggerKey enum validation, toggle/cancel state management, session recovery (encode/decode, restore, periodic save), NoteExporter (filename sanitization, markdown stripping, PDF rendering, security: path traversal/null bytes/shell metacharacters), AI provider error paths (malformed JSON, empty responses, 429/500/503 status codes), filler word boundary safety (umbilical/human/erase not stripped), backtrack edge cases (triple backtrack, unicode, document start), note store edge cases (search case-insensitivity, word count unicode/emoji/whitespace), keychain edge cases (empty/long keys, unicode, special chars)
